function DynamicBufferGeometry ( radius ) {
	...
	this.merge = function ( geometry ) {
		...
		if ( this.attributesKeys.length < 1 ) {
			// they are merging into empty object - create basic structure
			for ( var name in geometry.attributes ) {
				var attribute = geometry.attributes[ name ];
				this.addAttribute( name,
					new THREE.BufferAttribute( new attribute.array.constructor( 0 ), attribute.itemSize )
				);
			}

			if ( this.attributes.index == undefined ) {
				this.addAttribute( 'index',
					new THREE.BufferAttribute( new Uint16Array( 0 ), 1 )
				);
			}
		}
